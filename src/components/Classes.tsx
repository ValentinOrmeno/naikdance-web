        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {classesData.map((clase, index) => (
            <ScrollReveal key={clase.id} delay={index * 0.1}>
              <div className="group relative overflow-hidden rounded-2xl border border-white/10 hover:border-naik-gold transition-all duration-500 aspect-[4/5] cursor-pointer hover:shadow-[0_0_30px_rgba(255,215,0,0.3)]">
                <Image
                  src={clase.image}
                  alt={clase.name}
                  fill
